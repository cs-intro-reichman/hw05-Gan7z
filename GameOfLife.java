...
public class GameOfLife {
	...
	private static void test2(String fileName) {
		...
		System.out.println("==========================");
		int[][] countCheck = new int[board.length-2][board[0].length-2]; // shows the surrounding count of alive cells based on inital generation
		for (int i = 0; i < countCheck.length; i++){
			for(int j = 0; j < countCheck.length;j++){
				countCheck[i][j] = count(board, i+1, j+1);
			}
		}
		print(countCheck);
		System.out.println("=============");
		int[][] cellValueCheck = new int[board.length-2][board[0].length-2]; // checks the next generation, for checking purpose
		for (int i = 1; i < cellValueCheck.length; i++){
			for(int j = 1; j < cellValueCheck[0].length;j++){
				cellValueCheck[i][j] = cellValue(board, i+1, j+1);
			}
		}
		print(cellValueCheck);
	}
	...
	public static int[][] read(String fileName) {
		...
		for(int i = 1; i <= rows; i++ ){
			String reader = in.readLine(); // reads the data from the fileName
			int tempcount = reader.length(); // temporary count that used to read the 'reader' indexs
			for (int j = 0; j < tempcount; j++){
				if ( reader.charAt(j) == '.' ){
					board[i][j+1] = 0;
				}
				else if (reader.charAt(j) == 'x'){
					board[i][j+1] = 1;
				}
			}
		}
	...
	public static int[][] evolve(int[][] board) {
		...
		for (int i = 1; i < nextGenBoard.length-1; i++){
			for(int j = 1; j < nextGenBoard[0].length-1;j++){
				nextGenBoard[i][j] = cellValue(board, i, j);
			}
		}
		return nextGenBoard;
	}
